  ...
  return (
    <div className="py-16 bg-erb-gray-50">
      <div className="container">
        <h1 className={getJapaneseTextClass(locale, "text-4xl font-bold text-center mb-12 text-erb-navy")}>{t('title')}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {services.map((service) => (
            ...
            >
              ...
              <h2 className={getJapaneseTextClass(locale, "text-2xl font-bold mb-3 text-erb-navy")}>{service.title}</h2>
              <p className={getJapaneseTextClass(locale, "text-erb-gray-500 mb-4")}>{service.description}</p>
              
              <div className="space-y-3 mb-6">
                <div className="grid grid-cols-[80px,1fr] gap-2 items-start">
                  <span className="text-sm font-medium text-erb-navy">Price:</span>
                  <span className={getJapaneseTextClass(locale, "text-sm text-erb-primary font-medium")}>{service.price}</span>
                </div>
                <div className="grid grid-cols-[80px,1fr] gap-2 items-start">
                  <span className="text-sm font-medium text-erb-navy">Timeline:</span>
                  <span className={getJapaneseTextClass(locale, "text-sm text-erb-gray-500")}>{service.timeline}</span>
                </div>
                <div className="grid grid-cols-[80px,1fr] gap-2 items-start">
                  <span className="text-sm font-medium text-erb-navy">Includes:</span>
                  <span className={getJapaneseTextClass(locale, "text-sm text-erb-gray-500")}>{service.includes}</span>
                </div>
              </div>
            </div>
          ))}
